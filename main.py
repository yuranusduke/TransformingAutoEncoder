...
def get_data_loader(train : bool, batch_size : int) -> t.utils.data.DataLoader:
    """Get data loader
    Args :
...
def train(cap_dim : int, out_dim : int, num_caps : int, epochs : int, batch_size : int):
    """train the model
    Args :
        --cap_dim: capsule hidden dimension
        --out_dim: output dimension
        ...
        --batch_size
    """
    data_loader = get_data_loader(True, batch_size = batch_size)
    ...
    t.save(model, './results/model.pth')
...
def test(batch_size : int):
    ...
    Args :
        --batch_size
    """
    model = t.load('./results/model.pth')
    model.to(device)

    data_loader = get_data_loader(train = False, batch_size = batch_size)
    model.eval()
